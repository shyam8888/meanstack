$( document ).ready(function() {
	...
	$("body").on("focusout",".paper-input .form-control",function(){
    	if($(this).val() === ""){
		...
		var dcdropdown = function() {
			if ( $(window).width() < 767) {
				...
				dropdown.on('show.bs.dropdown', function(e){
					...
					var dropdowncenter = that.hasClass('dropdown-menu-center');
				...
				dropdown.on('hide.bs.dropdown', function(e){
					$dataSidebar = $(this).find('.dropdown-toggle').attr("data-sidebar");
					var dropdowncenter = $(this).find('.dropdown-menu').hasClass('dropdown-menu-center');
					var that = $(this).find('.dropdown-menu');
					var w = that.outerWidth();
					var h = that.outerHeight();
					var dcdmc = that.closest('.dropdown-menu-container');
	...
	//-- Radio Ripple Effect --//
	$(".radio-ripple-effect").on('mousedown', function(e) {
		var rippler = $(this);
		$('.ink').remove();
		// create .ink element if it doesn't exist
		if(rippler.find(".ink").length == 0) {
